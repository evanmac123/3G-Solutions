...
function jevelin_ocdi_import_files() {
    ...
    foreach( $newdemos as $demo ) :
        $thisdemo = array(
            ...
            'import_file_url'            => 'http://remote.demos.shufflehound.com/jevelin/ocdi_wpbakery_files/'.$demo['slug'].'/'.$demo['slug'].'_content.xml',
            'import_widget_file_url'     => 'http://remote.demos.shufflehound.com/jevelin/ocdi_wpbakery_files/'.$demo['slug'].'/'.$demo['slug'].'_widgets.wie',
            'import_json'                => array(
                array(
                    'file_url'    => 'http://remote.demos.shufflehound.com/jevelin/ocdi_wpbakery_files/'.$demo['slug'].'/'.$demo['slug'].'_options.json',
                    'option_name' => 'fw_theme_settings_options:jevelin',
                ),
            ),
            'import_preview_image_url'   => 'http://remote.demos.shufflehound.com/jevelin/files/'.$demo['image_url'].'.jpg',
            'preview_url'                => 'http://jevelin.shufflehound.com/'.$demo['preview_url'],
        );

        if( $demo['revslider'] ) :
            $thisdemo['import_revslider'] = array(
                array(
                    'file_url'    => 'http://remote.demos.shufflehound.com/jevelin/ocdi_wpbakery_files/'.$demo['slug'].'/'.$demo['slug'].'_slider.zip',
                ),
            );
        endif;
...
function jevelin_ocdi_after_import_setup( $selected_import ) {
    ...
    // fix onepager links
    $theme_urls = array(
        'http://cdn.jevelin.shufflehound.com/creative-vc/',
    );
